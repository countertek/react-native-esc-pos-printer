...
test('nested run after await throws on the thenable fallback', { timeout: 2000 }, async () => {
...
test('sequential run on the same Printer works after the thenable job settles', async () => {
...
test('sibling run after the first job has started waits on the thenable fallback', async () => {
  const order = [];
  let releaseFirst = () => {};
  let notifyStarted = () => {};
