...
Fetch the 2.37 zips, read `EULA.en.txt`, vendor binaries in `ios/` and `android/` only if that EULA allows shipping them inside a third-party npm library. If it does not, stop.
